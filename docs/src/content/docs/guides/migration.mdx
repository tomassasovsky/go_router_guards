...
Simple guards without boilerplate:

```dart
// Guard when condition is true
final featureGuard = guardWhen(
  () => featureFlags.isEnabled('premium'),
  '/upgrade',
);

// Guard when condition is false
final maintenanceGuard = guardUnless(
  () => isMaintenancMode(),
  '/maintenance',
);
```
...
testWidgets('guard redirects unauthenticated users', (tester) async {
  ...
  final result = await guard.executeWithResolver(context, mockState);
